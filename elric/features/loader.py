...
class KnowledgeBaseLoader(object):
    ...
    def load_entities(directory):
        return __read_file(path.join(directory, 'entity2id.txt'), ['entity', 'id'])

    def load_relations(directory):
        return __read_file(path.join(directory, 'relation2id.txt'), ['relation', 'id'])

    def load_facts(directory):
        return __read_file(path.join(directory, 'train2id.txt'), ['head', 'tail', 'relation'])
